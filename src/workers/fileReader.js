...
/**
 * Igual que leerArchivoCatalogo, pero SIN exigir columnas obligatorias fijas:
 * lo usan las importaciones de proveedores y categorías (Importaciones del
 * panel), donde cada fila se valida más adelante con las mismas reglas que
 * ya usa el formulario manual (helpers/proveedor.service.js y
 * helpers/categoria.service.js), fila por fila, no por el header completo.
 */
function leerArchivoGenerico(rutaArchivo) {
  const ext = path.extname(rutaArchivo).toLowerCase();
  const contenido = fs.readFileSync(rutaArchivo, 'utf-8');
...
module.exports = { leerArchivoCatalogo, leerArchivoGenerico };
