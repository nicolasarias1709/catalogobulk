...
function errorHandler(err, req, res, next) {
  ...
  // Un id con formato inválido (ej. "abc" en vez de un ObjectId de Mongo)
  // -> 400. Antes esto no se distinguía y caía en el 500 genérico de más
  // abajo, escondiendo que el problema era un id mal formado, no un bug.
  if (err.name === 'CastError') {
    return res.status(400).json({
      error: {
        codigo: 'ID_INVALIDO',
        mensaje: `Valor inválido para ${err.path}: "${err.value}"`,
      },
    });
  }
